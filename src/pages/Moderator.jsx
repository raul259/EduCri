...
function useTeacherProfiles() {
  ...
  async function reject(userId, notes) {
    const { error } = await supabase
      .from('teacher_profiles')
      .update({ approval_status: 'rejected', approval_notes: notes || null })
      .eq('user_id', userId)
    if (error) throw error
  ...
  async function changeRole(userId, newType) {
    const { error } = await supabase
      .from('teacher_profiles')
      .update({ teacher_type: newType })
      .eq('user_id', userId)
    if (error) throw error
    setProfiles(prev => prev.map(p => p.user_id === userId ? { ...p, teacher_type: newType } : p))
  }

  return { profiles, loading, approve, reject, changeRole, reload: load }
}
...
function PendingCard({ profile, onApprove, onReject }) {
  ...
  async function handleApprove() {
    setLoading(true)
    try { await onApprove(profile.user_id) } finally { setLoading(false) }
  }
    ...
    <div className="bg-white rounded-2xl border border-amber-200 shadow-sm p-5 space-y-3">
      ...
      <ul className="text-sm text-gray-600 space-y-1">
        ...
        <li className="flex items-start gap-2">
          <i className="fas fa-book-open text-gray-400 w-4 mt-0.5" />
          <span className="line-clamp-2">{profile.teaching_experience || '—'}</span>
        </li>
        <li className="flex items-center gap-2">
          <i className={`fas fa-shield-halved w-4 ${profile.has_cds ? 'text-green-500' : 'text-gray-300'}`} />
          {profile.has_cds
            ? `CDS válido${profile.cds_expiry_date ? ` hasta ${profile.cds_expiry_date}` : ''}`
            : 'Sin CDS'}
        </li>
      </ul>
      ...
      <div className="flex gap-2 pt-1">
        <button
          ...
          disabled={loading}
          className="flex-1 py-2 rounded-xl bg-green-500 text-white text-sm font-semibold hover:bg-green-600 transition-colors disabled:opacity-60"
        >
          <i className="fas fa-check mr-1.5" />Aprobar
        </button>
        <button
          onClick={() => onReject(profile)}
          disabled={loading}
          className="flex-1 py-2 rounded-xl bg-red-50 text-red-600 text-sm font-semibold hover:bg-red-100 transition-colors disabled:opacity-60"
        >
          <i className="fas fa-times mr-1.5" />Rechazar
        </button>
      </div>
    </div>
  )
...
function ActiveRow({ profile, onChangeRole }) {
  ...
  const nextLabel = TYPE_LABEL[nextType]
  const nextIcon  = TYPE_ICON[nextType]

  async function handleSwitch() {
    setLoading(true)
    try { await onChangeRole(profile.user_id, nextType) } finally { setLoading(false) }
  }

  return (
    <div className="flex items-center justify-between gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-50 text-blue-700 text-xs font-medium hover:bg-blue-100 transition-colors disabled:opacity-50 whitespace-nowrap flex-shrink-0"
      >
        {loading
          ? <i className="fas fa-spinner fa-spin" />
          : <><span>{nextIcon}</span> Pasar a {nextLabel}</>
        }
      </button>
    </div>
  )
...
export default function Moderator() {
  const { showToast } = useApp()
  const { profiles, loading, approve, reject, changeRole } = useTeacherProfiles()
  ...
  async function handleApprove(userId) {
    try {
      ...
      showToast('Profesor aprobado.', 'success')
    } catch {
  ...
  async function handleRejectConfirm() {
    ...
    setRejecting(true)
    try {
      await reject(rejectTarget.user_id, rejectNotes)
  ...
  async function handleChangeRole(userId, newType) {
    try {
      await changeRole(userId, newType)
      showToast(`Rol actualizado a ${TYPE_LABEL[newType]}.`, 'success')
    } catch {
  ...
  const TABS = [
    ...
    { key: 'active',   label: 'Aprobados',  count: active.length,   color: 'text-green-600' },
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
        {TABS.map(t => (
          ...
          >
            {t.label}
            <span className={`text-xs font-bold ${tab === t.key ? t.color : 'text-gray-400'}`}>
              {t.count}
            </span>
          </button>
        ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {pending.map(p => (
                  <PendingCard
                    ...
                    onReject={prof => { setRejectTarget(prof); setRejectNotes('') }}
                  />
                ))}
              </div>
            ) : (
          ...
          {tab === 'active' && (
            active.length ? (
              <div className="bg-white rounded-2xl shadow-sm divide-y divide-gray-100">
                {active.map(p => (
                  <ActiveRow key={p.user_id} profile={p} onChangeRole={handleChangeRole} />
                ))}
              </div>
            ) : (
          ...
          {tab === 'rejected' && (
            rejected.length ? (
              <div className="space-y-3">
                {rejected.map(p => (
                  <div key={p.user_id} className="bg-white rounded-2xl border border-red-100 p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-gray-700">{p.full_name || 'Sin nombre'}</p>
                        {p.approval_notes && (
                          <p className="text-xs text-red-500 mt-0.5">{p.approval_notes}</p>
                        )}
                      </div>
                      <button
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            ¿Rechazar la solicitud de <strong>{rejectTarget?.full_name}</strong>?
          </p>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">
              Motivo (opcional, se lo mostramos al profesor)
            </label>
            <textarea
...
function Empty({ icon, text }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm p-12 text-center text-gray-400">
      <i className={`fas ${icon} text-4xl mb-3 block`} />
